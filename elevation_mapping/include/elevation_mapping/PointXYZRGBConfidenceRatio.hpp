...
struct PointXYZRGBConfidenceRatio : public _PointXYZRGBConfidenceRatio {
  ...
  inline PointXYZRGBConfidenceRatio(
      float _x, float _y, float _z, std::uint8_t _r, std::uint8_t _g,
      std::uint8_t _b, float _confidence_ratio = 1.f) {
    x = _x;
    y = _y;
    z = _z;
    data[3] = 1.0f;
  ...
  friend std::ostream& operator<<(
      std::ostream& os, const PointXYZRGBConfidenceRatio& p);
};

PCL_EXPORTS std::ostream& operator<<(
    std::ostream& os, const PointXYZRGBConfidenceRatio& p);
...
POINT_CLOUD_REGISTER_POINT_STRUCT(
    pcl::_PointXYZRGBConfidenceRatio,  // here we assume a XYZ +
                                       // "confidence_ratio" (as fields)
    (float, x, x)                      // NOLINT
    (float, y, y)                      // NOLINT
    (float, z, z)                      // NOLINT
...
POINT_CLOUD_REGISTER_POINT_WRAPPER(
    pcl::PointXYZRGBConfidenceRatio, pcl::_PointXYZRGBConfidenceRatio)
